    ...
    private fun initFragment() {
        ...
        arrayOf(btn_tab_makeIdea, btn_tab_write).forEach {
            it.setOnClickListener {
                ...
                when (it) {
                    btn_tab_makeIdea -> {
                        btn_tab_makeIdea.setTextColor(ContextCompat.getColor(this@WordsActivity, R.color.colorPrimary))
                        btn_tab_write.setTextColor(0xFFBDBDBD.toInt())
                        line_review.visibility = View.INVISIBLE
                        line_detail.visibility = View.VISIBLE
                    }
                    else -> {
                        btn_tab_write.setTextColor(ContextCompat.getColor(this@WordsActivity, R.color.colorPrimary))
